...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase = 'test',split_comber=None):
        ...
        for idx in idcs:
            if config['luna_raw'] ==True:
                try:
                    l = np.load(os.path.join(data_dir, '%s_label_raw.npy' % idx), allow_pickle=True)
                except:
        ...
        # Clipping of patch data
        self.crop = Crop(config)
        # online hard negative mining (balanced positive and negative samples close to 7:3)
        self.label_mapping = LabelMapping(config, self.phase)

    def __getitem__(self, idx, split=None):
        # In the training phase, in the __getitem__() method, 
        # the returned result needs to perform crop operation 
        # and augment operation on the data, and the corresponding
        # label should also be transformed accordingly with the 
        # transformation of the image
        
        # In the test phase, cut directly
        t = time.time()
        ...
        # In the test phase, the original data needs to be cut 
        # (the split operation performs padding on the data, and processing on the z, x, and y axes)
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        if isScale:
            radiusLim = [8.,100.]
            scaleLim = [0.75,1.25] # An interval of image random scaling, which is equivalent to enhancing data
        ...
        # Image of crop [1, 128, 128, 128]
        ## target is the location information of the current nodule index
        ## bboxes is the location information of the current image
