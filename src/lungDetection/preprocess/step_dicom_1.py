...
"""
# Preprocessing Step1:
1) The first is to read the original CT data, which requires the sitk package. The read filename is in .mhd format, which contains the information of the CT image. In fact, the .raw file with the same name is also in the same directory;
2) Find the slice corresponding to the pulmonary nodule and circle the nodule;
"""
...
def get_pixels_hu(slices):
    ...
    a = [float(slices[0].SliceThickness)]
    return np.array(image, dtype=np.int16), np.array(a + list(slices[0].PixelSpacing), dtype=np.float32)
