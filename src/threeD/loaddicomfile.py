...
def load_scan(path):
    ...
    # thickness It is a blank value in dicom, calculate it and give it the value (the z difference between the two slices)
    try:
...
def get_pixels_hu(scans):
    ...
    # In the image where non-human body parts (surrounding air, etc.) are captured, the value will be -2000, set those to 0
    image[image == -2000] = 0
...
def make_mesh(image, threshold=-300, step_size=10):
    ...
    # Different transpose, the orientation of the graph is different, follow the convention to do (2,1,0)
    p = image.transpose(2, 1, 0)
    print('Calculating surface')
    # verts is all vertices, faces is a list of all triangles
